...
void insertCommandLoop(CmdNode *head, CmdNode *new_loop_node)
{
   if (head -> loop_head == NULL)
   {
      head -> loop_head = new_loop_node;
      return;
   }
   ...
   while (p != NULL)
   {
      ...
      else
      {
         ...
         while (x != NULL)
         {
            ...
            {
               if (strcmp(x->cmd,"forward")==0 || strcmp(x->cmd,"backward")==0 || strcmp(x->cmd,"color")==0)
               {
                  printf("%3d: %s %d\n",ctr, x ->cmd, x -> val);
               }
               else
      ...
      else
      {
         ...
         for (int i = 0; i < p -> val; i++)
         {
            ...
            while(a != NULL)
            {
               ...
               else if (strcmp(a -> cmd, "penup") == 0)
               {
                  if (write == 0) write = 1;
               }

               else if (strcmp(a -> cmd, "pendown") == 0)
               {
                  if (write == 1) write = 0; 
               }
               else if (strcmp(a -> cmd, "colour") == 0)
               {
                  if (a -> val >= 0 && a -> val < 256)
                  {
                     col = a -> val;
                  }
                  else if (a -> val > 255) col = 255;
                  else if (a -> val < 0) col = 0;
               }

               else if (strcmp(a -> cmd, "forward") == 0)
               {
                  if (turn == 0 && x + a-> val <= SIZEX-1 && x + a-> val >= 0)
                  {
                     x = x + a -> val;
                     if (write == 0) drawLine(im, x - a -> val, y, x, y, col); 
                  }

                  if (turn == 1 && y + a-> val <= SIZEX-1 && y + a-> val >= 0)
                  {
                     y = y + a -> val;
                     if (write == 0) drawLine(im, x, y - a -> val, x, y, col); 
                  }

                  if (turn == 2 && x - a-> val <= SIZEX-1 && x - a-> val >= 0)
                  {
                     x = x - a -> val;
                     if (write == 0) drawLine(im, x + a->val, y, x, y, col); 
                  }

                  if (turn == 3 && y - a-> val <= SIZEX-1 && y - a-> val >= 0)
                  {
                     y = y - a -> val;
                     if (write == 0) drawLine(im, x, y + a -> val, x, y, col); 
                  }
               ...
               else if (strcmp(a->cmd, "backward") == 0)
               {
                  if (turn == 0 && x - a -> val <= SIZEX-1 && x - a -> val >= 0)
                  {
                     x = x - a -> val;
